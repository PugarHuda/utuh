...
contract UtuhProperties {
    ...
    /// {SETTLEMENT}, copied once. A `bytes constant` is inlined at every use, and its one use in
    /// runtime code, {_proof}, put the harness 614 bytes over EIP-170. Read from storage, the literal
    /// lives only in initcode.
    ...
    function _proof(uint64 height) internal view returns (UtuhRegistry.EventProof memory p) {
